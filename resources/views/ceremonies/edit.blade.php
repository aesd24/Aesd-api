        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div class="container">
                    ...
                    <form action="{{ route('ceremonies.update', $ceremonie->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="datetime-local" name="periode_time" id="periode_time" class="form-control border border-gray-300 rounded px-4 py-2 w-full"
                                   value="{{ old('periode_time', optional($ceremonie->churches->first()->pivot)->periode_time ? date('Y-m-d\TH:i', strtotime($ceremonie->churches->first()->pivot->periode_time)) : '') }}" required>
                        ...
                        <div class="form-group mb-4">
                            ...
                            @endif
                            <small class="text-gray-500">Laissez vide si vous ne souhaitez pas changer le média actuel.</small>
                            @if ($ceremonie->media)
                                <div class="mt-2">
                                    <strong>Média actuel :</strong>
                                    @if (preg_match('/\.(jpg|jpeg|png|gif)$/i', $ceremonie->media))
                                        <img class="w-24 h-24 object-cover rounded" src="{{ asset('storage/media/' . basename($ceremonie->media)) }}" alt="Image actuelle">
                        ...
                        @foreach ($churches as $church)
                            <div class="form-group mb-2">
                                <input type="checkbox" name="churches[]" value="{{ $church->id }}" id="church_{{ $church->id }}" class="mr-2"
                                       {{ in_array($church->id, $ceremonie->churches->pluck('id')->toArray()) ? 'checked' : '' }}>
                                <label for="church_{{ $church->id }}" class="text-gray-700">{{ $church->name }}</label>
                            </div>
                        @endforeach

                        <!-- Bouton de soumission -->
                        <button type="submit" class=" px-4 py-2 rounded">Mettre à jour</button>
                    </form>
                </div>
            </div>
        </div>
